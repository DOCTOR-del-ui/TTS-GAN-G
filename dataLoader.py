...
class cmapss_load_dataset(Dataset):
    def __init__(self, idx, window):
        self.idx = idx
        self.window = window

        # 加载 npz 数据
        path = os.path.join("data", f"cmapsst{idx}_norm_truth_{window}_train.npz")
        npz = np.load(path)
    ...
    def __getitem__(self, index):
        ...
        # 变换成 (feat, 1, window)
        x = np.transpose(x, (1, 0))   # (feat, window)
        x = x.reshape(x.shape[0], 1, x.shape[1])  # (feat, 1, window)

        return x, f"{self.idx}_{self.window}"