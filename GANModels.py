...
class Generator(nn.Module):
    def __init__(self, seq_len=150, patch_size=16, channels=3, num_classes=9, latent_dim=100, embed_dim=10, depth=3,
                 num_heads=8, forward_drop_rate=0.5, attn_drop_rate=0.5):
...
class PatchEmbedding_Linear(nn.Module):
    def __init__(self, in_channels, patch_size, emb_size, seq_length):
        """
        in_channels: data channels (你的 dataset channels，比如 14)
        patch_size: how many time-steps per patch (e.g. 2, 8 等)
        emb_size: embedding dim (discriminator)
        seq_length: full width/time length (e.g. 48)
        """
        super().__init__()
        assert seq_length % patch_size == 0, f"seq_length {seq_length} must be divisible by patch_size {patch_size}"
        self.patch_size = patch_size
        ...
        self.projection = nn.Sequential(
            # expecting input shape: (b, c, h, w) with h usually=1, w = seq_length
            Rearrange('b c (h s1) (w s2) -> b (h w) (s1 s2 c)', s1=1, s2=patch_size),
            nn.Linear(patch_size * in_channels, emb_size)
        )
        self.cls_token = nn.Parameter(torch.randn(1, 1, emb_size))
        # positions length must equal num_patches + 1 (for cls token)
        self.positions = nn.Parameter(torch.randn(num_patches + 1, emb_size))

    def forward(self, x: Tensor) -> Tensor:
        b, _, _, _ = x.shape
        x = self.projection(x)  # -> (b, num_patches, patch_size*in_channels) -> (b, num_patches, emb_size)
        cls_tokens = repeat(self.cls_token, '() n e -> b n e', b=b)
        x = torch.cat([cls_tokens, x], dim=1)  # (b, 1 + num_patches, emb_size)
        x = x + self.positions  # broadcast ok
        return x
