...
#                 raise NotImplementedError('{} unknown inital type'.format(args.init_type))
    ...
    gen_net = Generator(seq_len=args.cmapss_window or args.img_size,
                    patch_size=args.patch_size,
                    channels=args.channels,
                    latent_dim=args.latent_dim,
                    embed_dim= args.g_window_size if hasattr(args,'g_window_size') else 10,
                    depth = [int(x) for x in str(args.g_depth).split(',')][0] if isinstance(args.g_depth,str) else args.g_depth
                   )
    #gen_net = Generator()
    print(gen_net)
    dis_net = Discriminator(in_channels=args.channels,
                        patch_size=args.patch_size,
                        emb_size=args.df_dim,
                        seq_length=args.cmapss_window or args.img_size,
                        depth=args.d_depth)
...
#     test_loader = data.DataLoader(test_set, batch_size=args.dis_batch_size, num_workers=args.num_workers, shuffle=True)
    ...
    train_set = cmapss_load_dataset(args.cmapss_idx, args.cmapss_window)
    print("train_set加载完成")
    ...
    print("数据形状", next(iter(train_loader))[0].shape)
...
# Add module in model saving code exp'gen_net.module.state_dict()' to solve the model loading unpaired name problem
    ...
    gen_plot(gen_net, args)
        
        
def gen_plot(gen_net, args):
    print("正在准备采样")
    ...
    all_fakes = []     # 存放所有 CPU 上的生成结果

    for i in range(0, total_samples, batch_size):
        cur_bs = min(batch_size, total_samples - i)
        print(f"生成第 {i}~{i+cur_bs} 条样本...")

        # 在 GPU 上生成这一批
        noise = torch.randn(cur_bs, latent_dim).cuda()
        fake = gen_net(noise)

        # 放 CPU，再处理维度
        fake = fake.float().cpu()        # (cur_bs, 14, 1, 48)

        fake = fake.squeeze(2)           # → (cur_bs, 14, 48)
        fake = fake.permute(0, 2, 1)     # → (cur_bs, 48, 14)

        all_fakes.append(fake.detach().numpy())   # 放 numpy CPU 内存
    ...
    # 拼接 → (12800, 48, 14)
    fake_sigs_np = np.concatenate(all_fakes, axis=0)
    print("最终生成形状:", fake_sigs_np.shape)

    # 保存
    save_filename = f"ddpm_fake_cmapsstttsgan{args.cmapss_idx}_{args.cmapss_window}.npy"
    save_dir = os.path.join("AdaptDiffTS", "OUTPUT", f"cmapsstttsgan{args.cmapss_idx}_{args.cmapss_window}")
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, save_filename)
    np.save(save_path, fake_sigs_np)
